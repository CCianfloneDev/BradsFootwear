...
/*
    Cole Cianflone
    Nov 7th, 2022
    Purpose: Script to process creating, updating, or deleting a sneaker.
*/
...
if ($_POST['command'] == "Update") {
    if (strlen($_POST['sneaker_name']) > 1 && strlen($_POST['sneaker_value']) > 1 && strlen($_POST['sneaker_description']) > 1) {
        ...
        $user_id_modify = filter_input(INPUT_POST, 'user_id_insert', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    ...
    // sanitze id
    $sneaker_id = filter_input(INPUT_POST, 'sneaker_brand_id', FILTER_SANITIZE_NUMBER_INT);
